...
public sealed class BepuPhysicsEngine : IPhysicsEngine
{
    ...
    private void CreateBucketWalls()
    {
        ...
        // Left wall (-X)
        AddStaticBox(
            new Vector3(_bucketMinX - wallThickness / 2, centerY, wallCenterZ),
            wallThickness, bucketDepth + wallThickness * 2, _bucketHeight);

        // Right wall (+X)
        AddStaticBox(
            new Vector3(_bucketMaxX + wallThickness / 2, centerY, wallCenterZ),
            wallThickness, bucketDepth + wallThickness * 2, _bucketHeight);

        // Front wall (-Y)
        AddStaticBox(
            new Vector3(centerX, _bucketMinY - wallThickness / 2, wallCenterZ),
            bucketWidth + wallThickness * 2, wallThickness, _bucketHeight);

        // Back wall (+Y)
        AddStaticBox(
            new Vector3(centerX, _bucketMaxY + wallThickness / 2, wallCenterZ),
            bucketWidth + wallThickness * 2, wallThickness, _bucketHeight);
    }
